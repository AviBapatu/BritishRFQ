...
@pytest.mark.asyncio
async def test_strict_forcing_rejection(client, session: Session):
    """
    ...
    bid_resp = await client.post("/api/bids", json=bid_payload, headers={"Idempotency-Key": "00000000-0000-0000-0000-000000000001"})
...
@pytest.mark.asyncio
async def test_complex_ranking_rejection(client):
    """
    ...
    rfq_id = (await client.post("/api/rfqs", json=rfq_data)).json()["id"]

    # L1 bid: Total = 5000
    b1 = await client.post("/api/bids", json={
